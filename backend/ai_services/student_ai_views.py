"""Clean student-facing AI views used by ai_services.urls."""
...
def build_chat_response(
    *,
    ...
    if course_id:
        # GraphRAG 失败时回落为友好提示，避免聊天入口直接报错。
        try:
            ...
            logger.error(
                build_log_message(
                    "chat.graph_rag.fail", course_id=course_id, error=exc,
                )
            )
            return {
                "reply": "服务暂时不可用，请稍后重试。",
...
def ai_profile_analysis(request):
    ...
    has_ability = AbilityScore.objects.filter(
        user=request.user, course_id=course_id
    ).exists()
    ...
    if not result.get("success", False):
        return error_response(
            msg=str(result.get("error") or "AI 分析服务暂时不可用"),
            code=500,
        )
...
def ai_resource_reason(request):
    ...
    fallback = {
        "reason": f"{resource.title} 与当前知识点相关，可用于补强薄弱环节。",
        "learning_tips": "建议先快速浏览核心内容，再结合练习进行巩固。",
    }
    if llm_facade.is_available:
        result = llm_facade.generate_resource_reason(
            resource_info=resource_info,
            student_mastery=mastery,
            point_name=point_name,
            course_name=resource.course.name if resource.course_id else None,
        )
    else:
        result = fallback
...
def ai_feedback_report(request):
    ...
    for exam_question in ExamQuestion.objects.filter(exam=exam).select_related(
        "question"
    ):
    ...
    if llm_facade.is_available:
        ...
        fallback["recommendations"] = (
            result.get("recommendations") or fallback["recommendations"]
        )
...
def ai_refresh_learning_path(request):
    ...
    existing_path = (
        existing_path.filter(course=course).first() if existing_path else None
    )
    preserved_count = 0
    removed_count = 0
    if existing_path:
        preserved_count = existing_path.nodes.filter(
            status__in=("completed", "active", "skipped", "failed")
        ).count()
    ...
    return success_response(
        data={
            ...
            "new_count": new_count,
            "change_summary": {
                "preserved_context": preserved_count,
...
def ai_time_scheduling(request):
    ...
    weak_points = (
        KnowledgeMastery.objects.filter(
            user=request.user,
            course_id=course_id,
            mastery_rate__lt=0.7,
        )
        .select_related("knowledge_point")
        .order_by("mastery_rate")
    )
    total_weight = sum(1 - float(record.mastery_rate) for record in weak_points) or 1.0
    schedule = []
    for record in weak_points[:8]:
        hours = round(
            available_hours * (1 - float(record.mastery_rate)) / total_weight, 1
        )
        schedule.append(
            {
                "knowledge_point": record.knowledge_point.name
    ...
    return success_response(
        data={
            ...
            "snapshot1": {
                ...
                "weakness": snapshot1.weakness,
                "suggestion": snapshot1.suggestion,
            }
            if snapshot1
            else None,
            "date2": date2,
            "snapshot2": {
                ...
                "weakness": snapshot2.weakness,
                "suggestion": snapshot2.suggestion,
            }
            if snapshot2
            else None,
        }
