...
class StudentLearningRAGContextTests(StudentLearningRAGFixture):
    ...
    def test_build_path_context_should_expose_multi_mode_sources(
        self, mock_ensure_index
    ):
        ...
        self.assertTrue(
            any(
                item["query_mode"] in {"local", "global", "drift"}
    ...
    @patch.object(student_learning_rag, "_ensure_index")
    def test_local_context_should_merge_vector_hits_into_sources(
        self, mock_ensure_index, mock_search_documents
    ):
        ...
        self.assertTrue(
            any(
                source.get("retrieval_source") == COURSE_RETRIEVAL_MODE
                for source in local_context.sources
            )
        )
    ...
    def test_answer_graph_question_should_merge_graph_query_sources(
        ...
        self.assertTrue(
            any(
                source.get("retrieval_source") == "text2cypher"
    ...
    def test_build_point_support_payload_should_include_graph_query_summary(
        ...
        self.assertTrue(
            any(
                source.get("query_mode") == "graph_tools"
                for source in result["sources"]
            )
        )
