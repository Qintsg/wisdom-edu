...
from .fixtures import StudentLearningRAGFixture
...
class StudentLearningRAGRuntimeTests(StudentLearningRAGFixture):
    ...
    def test_runtime_materialization_should_write_qdrant_points(
        ...
        class StubQdrantClient:
            ...
            def create_collection(
                self, collection_name, vectors_config, on_disk_payload
            ):
                """Record collection creation inputs for later assertions."""
                self.created_collections.append(
                    (collection_name, vectors_config, on_disk_payload)
                )
                return True
        ...
        artifact_report = student_graphrag_runtime.materialize_course_payload(
            101, payload
        )

        self.assertEqual(
            artifact_report["collection_name"],
            student_graphrag_runtime.collection_name(101),
        )
    ...
    def test_recommend_resources_for_node_should_return_internal_course_resources(
        self, mock_resource_filter
    ):
    ...
    @patch("platform_ai.rag.student.Resource.objects.filter")
    def test_recommend_resources_for_node_should_fallback_to_course_local_resources(
        self, mock_resource_filter
    ):
