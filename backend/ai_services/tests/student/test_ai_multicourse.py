...
class StudentAIMulticourseTests(APITestCase):
    ...
    @patch(
        "ai_services.services.student.graph_rag_service.student_graphrag_runtime.search_points"
    )
    def test_graph_rag_search_should_surface_runtime_supporting_sources(
        self, mock_runtime_search
    ):
        ...
        self.assertEqual(
            result["matched_points"][0]["supporting_sources"], ["课程A资源"]
        )
    ...
    @patch(
        "ai_services.services.student.graph_rag_service.student_learning_rag.answer_course_question"
    )
    def test_graph_rag_ask_should_route_structure_question_without_point(
        self, mock_answer_course_question
    ):
    ...
    @patch(
        "ai_services.services.student.graph_rag_service.student_learning_rag.answer_graph_question"
    )
    def test_graph_rag_ask_endpoint_should_surface_runtime_modes(
        self, mock_answer_graph_question
    ):
...
class _MCPStubSession:
    ...
    def post(
        self, url: str, headers: dict[str, str], json: dict[str, object], timeout: int
    ) -> _MCPStubResponse:
...
class ResourceMCPServiceTests(SimpleTestCase):
    ...
    def test_external_resource_mcp_should_search_tavily_with_single_key(self):
        ...
        self.assertEqual(
            stub_session.calls[0]["headers"]["Authorization"], "Bearer tvly-demo-key"
        )
    ...
    @patch(
        "platform_ai.search.providers.resource_mcp_service.search_external_resources"
    )
    ...
    def test_node_resource_recommendation_should_prefer_mcp_external_results(
        ...
        self.assertEqual(
            recommendation["external_resources"][0]["source"], "docs.example.com"
        )
