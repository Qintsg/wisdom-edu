...
class AppConfig:
    ...
    # 维护意图：返回 GraphRAG 的 Qdrant 本地持久化目录
    # 边界说明：调用契约在这里保持稳定，避免业务分支扩散到调用方。
    # 风险说明：调整调用契约时，需同步调用方、文档和回归测试。
    @staticmethod
