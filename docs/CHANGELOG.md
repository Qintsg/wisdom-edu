...
- `bootstrap-course-assets` 和 `pg-bootstrap` 的课程资源导入收尾现在会解析桌面的测评报告、学习画像、学习路径 HTML 及 `_files` 资源目录，为 `student1` 的“大数据技术与应用”自动补齐知识测评结果、初始答题历史、知识点掌握度、画像门禁状态和学习路径，路径完成到 `Spark SQL原理与特征基础`。
